...
class TestSaucedemoLoginFlowNotExist:
    ...
    def test_successful_login_and_inventory_navigation(self):
        ...
        Steps:
        1. Locate login form elements using resilient selectors
        ...
        """
        # Step 1: Locate login form elements using data-test attributes where available
        username_input = self.page.get_by_test_id("username")
        if not username_input.is_visible():
            username_input = self.page.locator("#user-name")
        
        password_input = self.page.get_by_test_id("password")
